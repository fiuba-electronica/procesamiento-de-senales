...
%% Matrices
Q = diag([sigmaAcel*h^2/2, sigmaAcel*h^2/2, sigmaAcel*h, sigmaAcel*h, ...
    ones(1,6)*0.005]); %Matriz de covarianza del ruido de proceso

R = eye(7)*sigma_d^2; %Matriz de covarianza del rudio de medicion

H = eye(10);
F = [eye(2) h*eye(2) zeros(2,6)
    zeros(2) eye(2) zeros(2,6)
    zeros(6,4) eye(6)];
D = [eye(2)*h^2/2
     eye(2)*h
     zeros(6,2)];
...
for k=1:length(time)
    ...
    %Actualizacion
    g = [
        dist(x(5:6)',x(1:2))      % dp,z1
        dist(x(7:8)',x(1:2))      % dp,z2
        dist(x(9:10)',x(1:2))     % dp,z3
    ...
    % Jacobiano
    G = [
        (x(1)-x(5))/g(1),(x(2)-x(6))/g(1),0, 0,-(x(1)-x(5))/g(1),-(x(2)-x(6))/g(1),0,0,0,0
        (x(1)-x(7))/g(2),(x(2)-x(8))/g(2),0, 0, 0, 0,-(x(1)-x(7))/g(2), -(x(2)-x(8))/g(2),0,0
        (x(1)-x(9))/g(3),(x(2)-x(10))/g(3),0, 0, 0, 0, 0, 0,-(x(1)-x(9))/g(3),-(x(2)-x(10))/g(3)
        (x(1)-xi(1,1))/g(4),(x(2)-xi(1,2))/g(4),0, 0, 0, 0, 0, 0, 0, 0
        (x(1)-xi(2,1))/g(5),(x(2)-xi(2,2))/g(5),0, 0, 0, 0, 0, 0, 0, 0
        (x(1)-xi(3,1))/g(6),(x(2)-xi(3,1))/g(6),0, 0, 0, 0, 0, 0, 0, 0
        (x(1)-xi(4,1))/g(7),(x(2)-xi(4,2))/g(7),0, 0, 0, 0, 0, 0, 0, 0 
        ];
...
plot(x(5,:),x(6,:),'bx')          %Punto z1hat
plot(x(7,:),x(8,:),'bx')          %Punto z2hat
plot(x(9,:),x(10,:),'bx')         %Punto z3hat

...
%% Ejemplo 1
%% Hallar un filtro pasa bajos con:
wp = pi/3;
ws = 2*pi/3;
deltap = 0.001;
deltas = 0.001;
...
% Respuesta temporal del pasabajos ideal con retardo de M/2.
hid = @(n,M,omegac) sinc(omegac/pi*(n-M/2))*omegac/pi;

%% Caso 1: ventana de Hamming, que da -53dB en la ventana de paso


M1 = round((8*pi)/Deltaw); % orden del filtro

fprintf('\n\n1) Ventana de Hamming:\n\n')
fprintf('El orden del filtro por ventana de Hamming es: %d\n\n',M1)
...
hid1 = hid(n,M1,omegac); % filtro ideal (truncado a 0:M)
f1 = w1.'.*hid1; % filtro obtenido
...
fprintf('\nEl filtro es pasabajos o sea que puede ser tipo II\n\n');
...
legend('Filtro ideal truncado a ventanear','Ventana de Hamming','Filtro obtenido por Hamming')
grid on
...
ylim([-80, 1])
grid on
legend('Respuesta del filtro por Hamming','Respuesta por ventana de Kaiser')
ylabel('Amplitud [dB]')
...
hold on
plot(omegan,20*log10(abs(F2)),'r','linewidth',2)
plot([0, wp/pi, wp/pi, 0],[maxpband,maxpband,minpband,minpband],'k--','linewidth',2)
xlim([0,wp/pi*1.2])
ylim([1.2*minpband, maxpband*1.2])
grid on
legend('Respuesta del filtro por Hamming','Respuesta por ventana de Kaiser','Tolerancias','location','northwest')
ylabel('Amplitud [dB]')
xlabel('Frecuencia normalizada')
...
hold on
plot(omegan,20*log10(abs(F2)),'r','linewidth',2)
plot([1, ws/pi,ws/pi],[deltadB,deltadB,-90],'k--','linewidth',2)
xlim([ws/pi*0.8,1])
ylim([-90, deltadB*0.8])
grid on
legend('Respuesta del filtro por Hamming','Respuesta por ventana de Kaiser','Tolerancias','location','northeast')
ylabel('Amplitud [dB]')
xlabel('Frecuencia normalizada')
